...
class Sheet:
    ...
    def append_data_to_range(self, data, spreadsheet_id, range_):
        ...
        values = [header_row] + [[row.get(key, '') for key in row] for sublist in data for row in
                                 sublist]
        ...
        return self.service.spreadsheets().values().append(spreadsheetId=spreadsheet_id,
                                                           range=range_,
    ...
    def write_data_to_range(self, data, range_, spreadsheet_id, types):
        import itertools
        types_list = list(types['type'].to_dict().values())
        header_row = list(data[0][0].keys())
        header_row.append('types')
        values = [[row.get(key, '') for key in row] for sublist in data for row in
                                 sublist]
